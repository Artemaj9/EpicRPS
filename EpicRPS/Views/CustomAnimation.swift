...
struct CustomAnimation: View {
    ...
    var body: some View {
        ZStack {
            ForEach(0..<total, id: \.self) { index in
                ...
                let extraRandomY: CGFloat =
                    (progress < 0.5 ? .random(in: 0...70) : .random(in: -70...0))

                Image(systemName: "star.fill")
                    .foregroundStyle(.red)
                    .scaleEffect(calculateScaleEffect(vm.strokeTime))
                    .offset(
                        x: (randomX + extraRandomX) * 0.8 * min(2, max(0, vm.strokeTime - 0.8)),
    ...
    private func calculateRandomY(progress: CGFloat, maxY: CGFloat) -> CGFloat {
        return ((progress > 0.5 ? progress - 0.5 : progress) * maxY) * CGFloat.random(in: -3...3)
            + 35
    }
